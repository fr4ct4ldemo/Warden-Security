...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('purgelinks').setDescription('Delete messages containing links')
  ...
  async execute(interaction, client) {
    try {
      ...
      const toDelete = messages.filter(m => urlRegex.test(m.content) && Date.now() - m.createdTimestamp < 1209600000);
      await channel.bulkDelete(toDelete, true);
      const embed = successEmbed('🔗 Link Messages Purged', `Deleted **${toDelete.size}** message(s) containing links in ${channel}.`);
