...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('purgebots').setDescription('Delete messages sent by bots in a channel')
  ...
  async execute(interaction, client) {
    try {
      ...
      const toDelete = messages.filter(m => m.author.bot && Date.now() - m.createdTimestamp < 1209600000);
      await channel.bulkDelete(toDelete, true);
      const embed = successEmbed('🤖 Bot Messages Purged', `Deleted **${toDelete.size}** bot message(s) in ${channel}.`);
