...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('rolebots').setDescription('Add or remove a role from all bots')
  ...
  async execute(interaction, client) {
    try {
      ...
      const embed = successEmbed(`🤖 Role ${action === 'add' ? 'Added to' : 'Removed from'} Bots`, `**${role.name}** ${action === 'add' ? 'added to' : 'removed from'} **${count}** bot(s).`);
