...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('purgeembeds').setDescription('Delete messages containing embeds')
  ...
  async execute(interaction, client) {
    try {
      ...
      const toDelete = messages.filter(m => m.embeds.length > 0 && Date.now() - m.createdTimestamp < 1209600000);
      await channel.bulkDelete(toDelete, true);
      const embed = successEmbed('📦 Embed Messages Purged', `Deleted **${toDelete.size}** message(s) with embeds in ${channel}.`);
