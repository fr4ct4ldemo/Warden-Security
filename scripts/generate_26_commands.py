...
commands = [
    ...
    },
    {
        ...
        'options': [
            "    .addStringOption(o => o.setName('messageid').setDescription('Message ID to unpin').setRequired(true))",
            "    .addChannelOption(o => o.setName('channel').setDescription('Channel containing the message'))"
        ],
        'body': [
            '      const channel = interaction.options.getChannel(\'channel\') || interaction.channel;',
...
for cmd in commands:
    lines = [header, 'module.exports = {', '  data: new SlashCommandBuilder()']
    ...
    lines.append('};')
    text = '\n'.join(lines) + '\n'
    cmd['path'].write_text(text, encoding='utf-8')
    print('Wrote', cmd['path'])
